...
//        $this->stateMap['ERROR_TYPE_NOT_ALLOWED'] = iconv( 'GB2312', 'UTF-8',$msg );
    ...
    private function saveRec( $arrFileInfo )
    {
        if( !$arrFileInfo )
        {
            echo 'Error!<br>';	
    ...
    private function upFile()
    {
        ...
        {
             include_once __DIR__ . '/fastdfs_upload.php';
        
            $arrFileInfo = uploadFile( $file["tmp_name"], trim( $this->fileType, '.' ) );
...
//         $this->fileName = $this->getFileName();
        ...
        if( $editorConfig[ 'type'] )
        {
            ...
            $arrFileInfo = uploadFileByStuff( $img, trim( $this->fileType, '.' ) );
            if( $arrFileInfo !== false )
            {
                $this->stateInfo = 'FAIL';
            }
