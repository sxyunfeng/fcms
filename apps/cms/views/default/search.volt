			...
			<div class="collapse navbar-collapse navbar-responsive-collapse self_nopadding" style="color:#e8e8e8;">
	        	...
	        	<?php 
					...
					{ 
						...
						{
				?>
			        <li class="<?php if( 0 == $i ) echo 'active'; ?> dropdown self_dropdown__<?php echo $i; ?>">
			          <a href="<?php if( false != $menu['url'] ) echo $menu['url']; else if( isset( $menu['relid'] ) && false == $menu['relid'] ) echo $this->menu->getListLink(); else if( isset( $menu['relid'] ) && false != $menu['relid'] ) echo $this->menu->getListLink( 'artcate', $menu['relid'] ); ?>" class="dropdown-toggle self_dropdown_toggle__<?php echo $i; ?>" target="<?php if( isset( $menu[ 'target' ] ) && 0 != $menu['target'] ) echo '_blank'; ?>" data-toggle="<?php if( isset( $menu['children'] ) && count( $menu['children'] ) > 0 ) echo 'dropdown'; ?>" role="button" aria-haspopup="true" aria-expanded="false"><?php echo $menu['name']; ?></a>
			          ...
			          <ul class="dropdown-menu self_dropdown_menu__<?php echo $i; ?>">
			          	...
			          	<li><a href="<?php if( false != $ch[ 'url' ] ) echo $ch[ 'url' ]; else if( isset( $ch['relid'] ) && false == $ch['relid'] ) echo $this->menu->getListLink(); else if( isset( $ch['relid'] ) && false != $ch['relid'] ) echo $this->menu->getListLink( 'artcate' , $ch[ 'relid'] );?>" target="<?php if( isset( $ch[ 'target' ] ) && 0 != $ch['target'] ) echo '_blank'; ?>"><?php echo $ch['name'];?></a></li>
			          	<?php } ?>
			          </ul>
			          <?php } ?>
			        </li>
			...
			?>
				<div class="self_artical" style="<?php if( 0 == $i ) echo 'border:none;'; ?>">
					...
					<div class="page-header self_search_pageheader">
						<h3><a href="<?php echo $this->article->getListLink( 'article', $row->sid ); ?>"><?php echo $row->title; ?></a></h3>
					</div>
					
					<div class="row">
						<?php if( false != $row->face ){ ?>
						<div class="col-md-3">
							<a href="<?php echo $this->article->getListLink( 'article', $row->sid ); ?>" class="thumbnail" style="margin-bottom:0;">
					...
					<div class="row self_search_footer">
						...
						<span>发布时间：<?php if( false != $row->time ) echo date( 'Y-m-d', strtotime( $row->time ) ); ?></span>
					</div>
				</div>
				<?php
						$i++;
