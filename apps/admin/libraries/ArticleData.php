...
use apps\admin\vos\ArticleCateVO;
...
class ArticleData implements InjectionAwareInterface{
	 ...
	 * @author( author='Carey' )
	 * @date( date = '2016-3-28' )
	 * @comment( comment = '取出当前文章相同tag的文章列表' )
	 * @method( method = 'getRelTagsArt' )
	 * @op( op = 'r' )
	 */
	public function getRelTagsArt( $aid )
	{
		$showNums = 5;//显示条数
		$arrIds = array();
		if( !$aid )
		...
		$where = array(
				'columns'	=> 'id,aid,delsign,uptime,addtime,name,pinyin,linkurl',
				'conditions'=> 'delsign=:del: and aid=:artid:',
				'bind'		=> array( 'del' => SystemEnums::DELSIGN_NO, 'artid' => $aid ),
		);
		...
		if( $tags && count( $tags ) > 0 )
		{
			foreach( $tags as $tag )
			{
				$tagWhere = array(
						'columns'	=> 'id,aid,delsign,uptime,addtime,name,pinyin,linkurl',
						'conditions'=> 'delsign=:del: and name=:tagname: and id <> :selid:',
						'bind'		=> array( 'del' => SystemEnums::DELSIGN_NO, 'tagname' => $tag->name, 'selid'=> $tag->id ),
				);
				$result = ArticleTags::find( $tagWhere );
				if( $result && count( $result ) )
				{
					foreach( $result as $row )
					{
						array_push( $arrIds , $row->aid );
					}
				}
			}
		}
		...
		{
			$artWhere = array(
					'columns'    => 'id,title,uptime,delsign',
					'conditions' => 'delsign=:del: and id in ('.implode(  array_unique($arrIds) , ',' ).') ORDER BY uptime desc LIMIT ' . $showNums,
					'bind'       => array( 'del' => SystemEnums::DELSIGN_NO )
			);
