...
<html>
    ...
    <body class="wrap">
        <ul class="nav nav-tabs" role="tablist">
            ...
            <li role="presentation" class="active"><a href="#listConfig">列表配置</a></li>
            <li role="presentation"><a href="/admin/staticcache/detail">详细页配置</a></li>
        </ul>
        <div class="tab-content" style="padding:20px 0px;">
            <div role="tabpannel" class="tab-pane active" id="listConfig">
                <table class="table table-hover table-bordered">
                    <thead>
                    	<tr>
                    		<th colspan="7"></th>
                    		<th style="text-align:center"><a href="/admin/staticcache/optlist"><i class="fa fa-plus-square"> 添加配置项</i></a></th>
                    	</tr>
                        <tr>
                        	<th>序号</th>
                            <th>编号</th>
                    ...
                    <tbody>
                        ...
                        <tr id="listCfg{{item.id}}">
                           ...
                           <td data-id = "{{ item.id | escape_attr }}" class="skinimg" style="padding-left:4%">
                                ...
                                <a href="{{ url( '/admin/staticcache/optlist/id/' ) }}{{ item.id | escape_attr }}"><font size="4"><i class="glyphicon glyphicon-pencil operate" title="修改"></i></font></a>
                            </td>
                        </tr>
                        {% endfor %}
                    </tbody>
                ...
                <nav class="text-right" >
                    <ul class="pagination pagination-sm" style="margin-top:0"> 
                        <li class="{% if page.current == 1 %}disabled{% endif %}"><a href="{{ url( 'admin/staticcache/list?page=' ) }}{{page.before | escape_attr }}" >&laquo;</a></li>
                        {% if  1 != page.current and 1 != page.before %}
                        <li><a href="{{ url( 'admin/staticcache/list') | escape_attr }}">1</a></li>
                        {% endif %}
                        {% if page.before != page.current  %}
                        <li><a href="{{ url( 'admin/staticcache/list?page=') }}{{ page.before | escape_attr }}"><span >{{ page.before |e}}</span></a></li>
                        {% endif %}
                        <li class="active"><a href="{{ url( 'admin/staticcache/list?page=') }}{{ page.current | escape_attr }}"><span >{{ page.current |e}}</span></a></li>
                        {% if page.next != page.current %}
                        <li><a href="{{ url( 'admin/staticcache/list?page=') }}{{ page.next | escape_attr }}">{{ page.next |e}}</a></li>
                        {% endif %}
                        {% if page.next  < page.last - 1 %}
                        <li><a>...</a></li>
                        {% endif %}
                        {% if page.last != page.next %}
                        <li><a href="{{ url( 'admin/staticcache/list?page=') }}{{ page.last | escape_attr }}">{{ page.last |e}}</a></li>
                        {% endif %}
                        <li class="{% if page.current == page.last %}disabled{% endif %}"><a href="{{ url( 'admin/staticcache/list?page=' ) }}{{page.next | escape_attr }}" >&raquo;</a></li>
        ...
        $(function(){
        	...
        	$( 'i.cacheDelete' ).click(function(){
	        	...
	        	$.get( '/admin/staticcache/delList/id/' + optid , function( ret ){
	        		if( 1 != ret.state )
					{
						$( '#listCfg' + ret.optid ).remove();
