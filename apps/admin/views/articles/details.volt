...
<title>
	...
	<?php if( false != $article && count( $article ) > 0 ) echo $article->title; ?>
</title>
</head>
<body>
	<!-- 返回顶部 -->
			...
			<div class="collapse navbar-collapse navbar-responsive-collapse self_nopadding" style="color:#e8e8e8;">
	        	...
	        	<?php
					...
					{ 
						...
						{
				?>
			        <li data-id="<?php echo $menu['id']; ?>" class="<?php if( false != $article && false != $menu['relid'] && $article->cat_id == $menu['relid'] ) echo 'active'; ?> dropdown self_dropdown__<?php echo $i; ?>">
			          <?php  if( false != $menu['url'] && isset( $menu['relartid'] ) ){ ?>
			          <a data-aid="<?php echo $menu['relartid']; ?>" href="<?php echo $menu['url'];?>" class="dropdown" target="<?php if( isset( $menu[ 'target' ] ) && 0 != $menu['target'] ) echo '_blank'; ?>" data-toggle="<?php if( isset( $menu['children'] ) && count( $menu['children'] ) > 0 ) echo 'dropdown'; ?>" role="button" aria-haspopup="true" aria-expanded="false"><?php echo $menu['name']; ?></a>
			          ...
			          <ul class="dropdown-menu self_dropdown_menu__<?php echo $i; ?>">
			          	...
			          	<li data-pid="<?php echo $ch['pid'];?>">
			          		<?php if( false != $ch[ 'url' ] && false != $ch[ 'relartid' ] ){ ?>
