...
class Verify implements \Phalcon\DI\InjectionAwareInterface{
    protected $config =	array(
        'seKey'     =>  'junhankj.com',   // 验证码加密密钥
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
//         $secode = session($key);
        $secode = $this->getDI()->get( 'session' )->get( $key );
      
        if(empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
        if( $this->NOW_TIME - $secode['verify_time'] > $this->expire) {
            $this->_di->get( 'session' )->set($key, null);
            return false;
        }

        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && $this->_di->get( 'session' )->set($key, null);
            return true;
        }
...
//         session($key.$id, $secode);
        $this->_di->get( 'session' )->set( $key.$id, $secode);
